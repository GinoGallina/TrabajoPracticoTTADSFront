...
export class CreatePaymentTypesComponent {
  ...
  onSubmit() {
    if (this.paymentTypeForm.valid) {
      this.paymentTypeService.createPaymentType(this.paymentTypeForm.value).subscribe(
        (res) => {
          this.notificationService.showSuccessNotification('Payment type created successfully');
          this.router.navigate(['/payment_type']);
        },
        (error) => {
          //VER DE CAMBIAR ESTO O NO
          this.notificationService.showErrorNotification(`Failed to create Payment type - ` + error.error.error[0].message);
        }
      );
